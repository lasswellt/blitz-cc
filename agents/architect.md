...
maxTurns: 15
# Sonnet by default per /_shared/agents.md routing matrix. For genuinely
...
> **Invocation:** Spawned via the orchestrator's freeform delegation (Agent tool) for structural-analysis requests — NOT wired into any `/blitz:*` skill (codebase-map spawns `general-purpose` dimension agents, not this agent). If the orchestrator is disabled (via a `{"agent": null}` user settings override — the `BLITZ_DISABLE_ORCHESTRATOR` env var is not currently honored by any blitz hook), reach it only through an explicit `Agent(subagent_type: "architect")` call from a skill context.
...
- Cross-reference with [Definition of Done](/_shared/quality.md).
...
- Return findings as your final text response — the orchestrator extracts them; never write files (READ-ONLY, per [/_shared/agents.md](/_shared/agents.md) §Agent Output Contract)
