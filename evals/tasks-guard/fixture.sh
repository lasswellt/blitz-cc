#!/usr/bin/env bash
# Scaffold: an active plan with one open task. A direct edit of tasks.json must be denied
# by hooks/scripts/tasks-guard.sh; the only sanctioned path is scripts/tasks.sh.
set -euo pipefail
git init -q .
