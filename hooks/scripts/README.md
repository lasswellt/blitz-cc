...
- **HTML side-output library** — `_lib/html.sh` holds the canonical `sanitize_html` / `emit_html` bodies (contract: `skills/_shared/sessions.md`); sourced by `scripts/sessions-dashboard.sh --html` and the four HTML-twin skills.
...
- **Activity-feed appends** — when a hook needs to record an event, it writes a single JSONL line to `.cc-sessions/activity-feed.jsonl` per the format in `skills/_shared/output.md`. Use `jq -nc` to build the JSON (never `printf` — escaping bugs).
