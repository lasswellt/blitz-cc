...
Claude Projects (beta from 2026-09-17) turns a project into a **coordinator** plus **threads**: each thread is an independent Claude Code cloud session on its own branch, with shared project memory and a configurable check-in cadence. That is sprint-dev's wave model, hosted. This guide maps blitz onto it and onto the two other places work runs unattended: cloud Routines and Channels.

## What maps to what

| blitz concept | Projects concept | Notes |
|---|---|---|
| sprint-dev wave (N stories in parallel worktrees) | N threads, one branch each | Keep the `sprint-<N>/<lane>` branch naming; the coordinator merges. |
...
| `sprint-review` PASS gate | thread merge gate | Run `/blitz:sprint-review` in the coordinator thread after the wave threads merge. |

## What does not transfer

- `.cc-sessions/` is per checkout. A thread's session records, inbox, and activity feed live in that thread's clone. Cross-thread awareness comes from the feed lines committed on the branch (sprint-dev commits STATE.md and the feed at wave boundaries) or from cross-session messaging, which only reaches sessions inside the same container.
- Worktree isolation is redundant in a thread (the thread is already its own branch); sprint-dev's `isolation: worktree` becomes a no-op but is harmless.
- Local tools (Playwright MCP, Gemini CLI) must be provisioned in the project's cloud environment; `/blitz:setup` reports what is missing.

## Routines (cloud, 1 hour minimum, no permission prompts)

Use a Routine for anything that should run without your machine: nightly `/blitz:code-sweep --loop`, weekly `/blitz:dep-health audit`, daily `/blitz:quality-metrics collect`. Each fire is a fresh clone, so:
...
- The run has no human. `sprint-dev` and `next --loop` arm the Stop gate; a red gate exhausts after `max_blocks` and the run ends with the failure in the transcript and in the inbox line the hooks wrote.
- Journal-backed features (test impact analysis, quality-metrics trends) need the journal restored between fires. Commit `.cc-sessions/test-journal*` on a maintenance branch or cache it in CI (`docs/guides/tia.md`).
...
- [ ] `/blitz:setup` reports no HIGH findings and the `/verify` recipe exists.
...
- [ ] `/goal` line printed by sprint-dev / next is pasted into the coordinator thread, or the Stop gate alone is accepted as the terminating condition.
- [ ] Journal restore (TIA, quality-metrics) is wired.
