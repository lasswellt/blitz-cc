...
Implementation (registry rows, `stack` probe code, skill edits, file deletion) is the follow-up sprint behind Blitz's gates (sequenced in archived design history; see pre-v2.4.1 git history). This doc specifies *what* changes and *why*; the sprint does it under sprint-review's 8-invariant gate.
