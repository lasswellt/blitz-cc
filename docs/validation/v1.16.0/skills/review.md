...
`quality-engine.md` line 27 documents the relationship: `review` is "(alias) — thin wrapper — flag-parses + forwards to sprint-review". Bidirectionality: `sprint-review/SKILL.md` does not explicitly cite `review` by name, but `quality-engine.md` (the shared authority) documents the relationship bidirectionally. No owned logic is restated in the wrapper; finding-format in `references/main.md` is wrapper-specific, not a restatement of sprint-review internals.
...
- Story files in `sprints/sprint-${N}/stories/` produced by sprint-plan; status transitioned to `done` by sprint-dev Phase 4.8 (session-lifecycle.md line 69). Review skill validates at least one story with `status: done` (SKILL.md line 39). Matches exactly.
...
All stated inputs match documented session-lifecycle.md producer outputs. No phantom consumption.
...
All runnable checks pass. Frontmatter validated by hook script. OUTPUT STYLE matches canonical byte-for-byte. All links resolve. Delegation to sprint-review is clean and non-duplicating. Pipeline I/O aligns with session-lifecycle.md. Body is 51 lines (well under budget).

---

## Highest-Leverage Fix

`sprint-review/SKILL.md` does not cite the `review` wrapper by name; the bidirectional relationship exists only in `quality-engine.md`. Adding a one-line note to `sprint-review/SKILL.md` (e.g., "Entry-point alias: `/blitz:review` (thin wrapper in `skills/review/SKILL.md`)") would make the delegation fully self-documenting without relying on the matrix as a cross-reference intermediary. Low-risk, no functional impact.
