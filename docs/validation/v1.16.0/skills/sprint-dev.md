...
**Evidence:** `hooks/scripts/markdown-link-validate.sh skills/sprint-dev/SKILL.md` output: `markdown-link-validate: OK (397 link(s) checked)`. All `/_shared/X` links in the Additional Resources section (lines 16-24) and in-body citations (`session-lifecycle.md`, `sprint-contracts.md`, `session-lifecycle.md`, `session-lifecycle.md`, `sprint-contracts.md`, `session-lifecycle.md`, `sprint-contracts.md`, `agent-orchestration.md`, `security.md`, `worktree-lifecycle.md`, `knowledge-protocol.md`, `terse-output.md`) resolve. `references/main.md` also exists at `skills/sprint-dev/references/main.md` (786 lines).
...
**Evidence:** `sprint-dev` is not a declared O1-O5 canonical owner and does not claim to own any shared protocol. It delegates to owners by citation — e.g., `agent-orchestration.md` for agent spawning, `sprint-contracts.md` §Writers for registry writes, `worktree-lifecycle.md` for cleanup contract — and does NOT restate the owned logic inline. Phase 4.4 explicitly says "Canonical contract: `/_shared/worktree-lifecycle.md`" and "Full cleanup script in `references/main.md`" rather than duplicating the logic here. Phase 4.7 likewise says "per `session-lifecycle.md` §File-Based Locking Protocol (the canonical acquire/verify/release sequence lives there — do not restate it)". Consumer-side bidirectional check: `session-lifecycle.md` §sprint-dev table (lines 63-73) references sprint-dev as the consumer of sprint-plan outputs and producer of `STATE.md` + story status transitions — confirmed present in `_shared/session-lifecycle.md` lines 63-73.
...
Per `_shared/session-lifecycle.md`:
...
- **sprint-dev consumes** exactly those artifacts: Phase 0.0 gate (SKILL.md lines 58-72) checks `sprint-registry.json`, `${SPRINT_DIR}/manifest.json`, and `${SPRINT_DIR}/stories/S*.md` — matching the three required inputs listed in `session-lifecycle.md` §sprint-plan.
- **sprint-dev produces** (session-lifecycle.md lines 63-73): `STATE.md`, story `status` transitions, `.cc-sessions/carry-forward.jsonl progress` lines, commits+branches. SKILL.md Phase 1b (line 294) writes carry-forward; Phase 4.8 (line 411) updates story frontmatter; Phase 3.2 step 1b (line 295) writes STATE.md. All outputs cited in handoff table are produced.
- **sprint-review consumes** those same artifacts per session-lifecycle.md §sprint-review. Composition is sound.

Story-frontmatter contract: Phase 0.0 (line 74) validates every story file against `sprint-contracts.md` §Validation algorithm. Fields extracted at Phase 1.3 (`id`, `title`, `assigned_agent`, `depends_on`, `priority`, `points`, `files`) match the canonical schema producer/consumer matrix.
...
**Evidence:** `agent-orchestration.md` line 83 explicitly classifies `sprint-dev` as **`deferred`** — not a current DW adopter. The skill has no `Workflow` dispatch gate. N/A is the correct verdict; no wiring defect to evaluate here.
...
**Verdict:** FAIL — drift from canonical Agent() pattern, no explicit blessing in agent-orchestration.md
...
4. `skills/_shared/agent-orchestration.md` line 79 (Foot-Guns section §1.3, item 5): **"`TeamCreate`+`SendMessage` does not accept `subagent_type`** — the SDK picks by heuristic. **Use the `Agent` tool instead** (v1.4.0 migrated all spawning skills to this)."** This is an explicit foot-gun warning and migration notice, not a blessing.
5. No section in `agent-orchestration.md` carves out an exception for `sprint-dev` or any other skill to use `TeamCreate` for team management while also using `Agent()` for spawning.
