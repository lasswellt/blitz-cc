...
Script validates: name, description (≤1024 chars, 482 chars actual), model (sonnet ✓ per token-budget.md matrix for standard workers), tools (Read, Write, Edit, Bash, Glob, Grep, WebSearch, ToolSearch), maxTurns (50), forbidden fields absent, body lines (194 ≤ 500), OUTPUT STYLE snippet present.
...
### ≤50-word JSON reply contract (token-budget.md §3)

**FAIL — missing from both the agent file and its authoritative spawn prompt.**

token-budget.md §3 (line 73): "Every Agent() prompt MUST instruct the subagent to return ONLY the canonical JSON shown below."
...
This is a systemic gap shared by `frontend-dev` and `test-writer` (grep across all writer agents confirms zero occurrences of "Return ONLY" or "files_changed" in those files). The sprint-dev orchestrator parses `DONE:` prefix messages, not JSON — a deliberate but undocumented deviation from the token-budget.md mandate.
