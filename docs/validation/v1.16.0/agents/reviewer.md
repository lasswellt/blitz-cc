...
- model: `sonnet` (matches agent-orchestration.md matrix — standard worker)
...
**JSON reply contract (agent-orchestration.md §3)**: reviewer is a leaf worker (does not spawn agents via Agent()); the JSON reply contract applies to callers invoking it via Agent(), not to the agent body itself. The agent produces a Markdown findings file (written incrementally, referenced path returned). No violation.

**Agent Output Contract (agent-orchestration.md §8)**: reviewer writes output to `${SESSION_TMP_DIR}/review-findings.md` or `/tmp/review-findings.md` (lines 39–48). Write-as-you-go protocol declared (lines 36–49). These satisfy the Medium-class mandatory patterns. No contract violation.
...
Write IS present. The reviewer is intentionally given Write access to produce findings files; agent-orchestration.md §1 table explicitly notes reviewer as "No (Write for findings only)".
...
**Contrast with architect** (agent-orchestration.md §1 table): `blitz:architect` is described as "strictly read-only" and lists only `Read, Glob, Grep, Bash` — Write is absent, providing structural enforcement. The reviewer does not have an equivalent structural guardrail.
