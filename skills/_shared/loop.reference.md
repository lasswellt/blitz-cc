...
Detail split out of [loop.md](loop.md) so the contract every skill loads stays small. Artifacts, the `tasks.json` schema, and the structural rules every skill must obey lives there; everything below is loaded on demand.
