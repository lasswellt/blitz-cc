...
|---|---|---|---|---|---|---|
| **sprint-review** | one sprint | per-sprint | type-check + lint + test + build + 4 parallel reviewer agents (security/backend/frontend/patterns) + critic agent | PASS/FAIL + 7-invariant report (`.cc-sessions/sprints/<sprint>/review.md`) | critic agent, reviewer/architect agents | ship; `/blitz:review` alias; manual |
...
|---|---|
| sprint-review + completeness-gate | Different concerns. sprint-review = "does this sprint pass the 7 invariants" (types, lint, tests, build, reviewer findings). completeness-gate = "are there placeholders". Chained sequentially by `ship`, not by each other. No code duplication. |
...
```
/blitz:ship
├── /blitz:sprint-review     (PASS gate — 7 invariants)
...
- `skills/sprint-review/SKILL.md` — 7-invariant gate spec
...
- `_shared/ratchet-protocol.md` — 7 monotonic metrics, multi-agent worktree merge, auto-revert
