...
Detail split out of [agents.md](agents.md) so the contract every skill loads stays small. The agent roster, the model-routing matrix, and when spawning is warranted at all lives there; everything below is loaded on demand.
...
Every `dev` / `test-writer` spawn carries all eleven items. **Items 6, 7, 9, 10 and 11 arrive through the hook**, not the prompt: `hooks/scripts/subagent-context.sh` injects [spawn-invariant.md](spawn-invariant.md) on `SubagentStart`, so they are byte-identical per spawn and stay cached. `build` assembles the rest from `tasks.json`. A spawn missing one of items 1–5 or 8 is a bug in the skill, not in the agent. When the hook is disabled (`BLITZ_DISABLE_SPAWN_INVARIANT=1`) or the spawn is not a blitz agent, inline the invariant content instead.
...
- **Blitz registers no `WorktreeCreate` hook, and must not.** Configuring one *replaces* the platform's `git worktree` creation entirely: the hook owns the checkout, must print the created directory as the last non-empty line of stdout, and "if the hook fails or produces no path, worktree creation fails with an error". A configured hook also makes the platform skip [`.worktreeinclude`](https://code.claude.com/docs/en/worktrees#copy-gitignored-files-into-worktrees). There is no observe-only mode. The event's only extra input field is `name` (a slug); `worktree_path` and `branch` do not exist on it. Register `WorktreeCreate` only to drive a non-git VCS, and then implement the whole checkout.
- `WorktreeRemove` **is** additive for git worktrees (the platform still runs `git worktree remove`). Its input carries the real `worktree_path`. A non-zero exit **fails the removal** when the directory still exists afterwards, so `worktree-remove.sh` always exits 0 and does its branch cleanup best-effort.
- Stale-branch collision (GH#51596: an 8-hex `worktree-agent-<id>` branch reused from a prior session) is a **pre-flight** check, not a creation veto: `doctor` D-314 and `build` Phase 0.4 scan `git for-each-ref` before a wave dispatches. `BLITZ_ALLOW_WORKTREE_COLLISION=1` downgrades it to a warning.
