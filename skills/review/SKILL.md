---
name: review
description: "Per-change precision review gate: deterministic lane (tsc/lint/test/build + grep/git/import-graph) + semantic lane (parallel reviewer agents), FP-verifies, gates by confidence, invokes the critic. Folds --only completeness|wiring|framework; delegates the full 8-invariant gate to /blitz:sprint-review. Use for pre-commit review of a diff or working tree: 'run quality gates', 'check completeness', 'check wiring', 'validate before shipping'. For the end-of-sprint PASS/FAIL gate use /blitz:sprint-review instead."
argument-hint: "[--sprint N] [--auto-fix] [--only completeness|wiring|framework|design|full] [--min-confidence high|low] [--dual]"
...
**Verbose progress is mandatory.** Follow [terse-output.md](/_shared/output.md). Print `[review]` status at every phase + dispatch. Log `skill_start`/`skill_complete` to the activity feed.
...
- `--min-confidence {high|low}` — advisory-finding gate band (default `high` ≥0.8). `reject`-authority findings always surface (bypass the gate). See [quality-engine.md](/_shared/quality.md).
...
Per [quality-engine.md](/_shared/quality.md), select `consolidated_target ∈ {review, both}`:
