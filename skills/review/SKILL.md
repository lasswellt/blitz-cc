...
- `--only design`: **first run the preflight** `scripts/design/preflight.sh <target-repo>` and render its `DESIGN_LANE_STATUS` banner. If `semantic != OK`, surface the `DESIGN_LANE_UNAVAILABLE` line in the summary, run only the lanes that don't need impeccable (the deterministic regex rows), and do **not** return green for the design pillar — coverage is reduced, not passed. Then resolve the adapter from `scripts/detect-stack.sh`; select `pillar == design` rows where `adapter ∈ {universal} ∪ {primary} ∪ secondary` and not suppressed by `reconciliation.relaxFor`. Vendored (semantic) rows share ONE impeccable run and match hits by `detection.filter`; deterministic L1/L2 rows run their `regex` command (always available, key-free). Layer 0 always on; Layer 1/2 gated to the stack. impeccable is a dependency of the **target project**, not the plugin (`npm i -D impeccable@2.3.2`). See [detector-rebuild.md](../../docs/integrations/impeccable/detector-rebuild.md) and [improvements/dependency-resolution.md](../../docs/integrations/impeccable/improvements/dependency-resolution.md).
