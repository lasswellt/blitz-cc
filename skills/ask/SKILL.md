...
**Verbose progress is mandatory.** Follow [terse-output.md](/_shared/output.md) throughout. Print `[ask]` prefixed status lines showing classification decisions, clarification steps, and dispatch targets. Log `skill_start` and `skill_complete` events to the activity feed (`.cc-sessions/activity-feed.jsonl`).
